...
cdef class CFW_D_Similarity_Cython_SGD:
    ...
    def __init__(self, row_list, col_list, data_list,
        ...
        self.row_list = np.array(row_list, dtype=int)
        self.col_list = np.array(col_list, dtype=int)
        self.data_list = np.array(data_list, dtype=float)
        ...
        self.common_features_id = np.zeros((self.n_features), dtype=int)
        self.common_features_data = np.zeros((self.n_features), dtype=float)
        self.common_features_data_i = np.zeros((self.n_features), dtype=float)
        self.common_features_flag = np.zeros((self.n_features), dtype=int)
        ...
        if self.precompute_common_features:
            ...
            self.icm_data = np.array(ICM.data, dtype=float)
        ...
        self.dropout_mask = np.ones(self.n_features, dtype=int)
    ...
    def run_tests(self):
        ...
        self.common_features_id = np.zeros((n_test_features), dtype=int)
        self.common_features_data = np.zeros((n_test_features), dtype=float)
        self.common_features_data_i = np.zeros((n_test_features), dtype=float)
        self.common_features_flag = np.zeros((n_test_features), dtype=int)
    ...
    def precompute_common_features_function(self, ICM):
        ...
        self.commonFeatures_indices = np.array(commonFeatures.indices, dtype=int)
        self.commonFeatures_indptr = np.array(commonFeatures.indptr, dtype=int)
        self.commonFeatures_data = np.array(commonFeatures.data, dtype=float)
    ...
    def fit(self):
        ...
        cdef long dropout_threshold = np.long(RAND_MAX * self.dropout_perc)
    ...
    def feature_common_test(self):
        ...
        # No duplicates
        f_i = np.array([0, 5, 8, 10, 19], dtype=int)
        f_j = np.array([1, 7, 9, 11, 15, 18], dtype=int)

        common_np = np.intersect1d(f_i, f_j)
        count = self.feature_common_unordered(f_i, f_j, np.ones_like(f_i, dtype=float), np.ones_like(f_j, dtype=float))
        ...
        # Duplicates
        f_i = np.array([0, 5, 8, 10, 19], dtype=int)
        f_j = np.array([1, 10, 11], dtype=int)
        ...
        f_i = np.array([0, 5, 8, 10, 19], dtype=int)
        f_j = np.array([1, 2, 5, 10, 15], dtype=int)
        ...
        for _ in range(100):
            ...
            f_i = np.random.randint(0,50, size=size_f_i, dtype=int)
            f_i = np.unique(f_i)

            size_f_j = np.random.randint(1,10)
            f_j = np.random.randint(0,50, size=size_f_j, dtype=int)
            f_j = np.unique(f_j)
