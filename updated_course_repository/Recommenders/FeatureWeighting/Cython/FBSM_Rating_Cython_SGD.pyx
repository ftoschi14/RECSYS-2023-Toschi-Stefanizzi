...
cdef class FBSM_Rating_Cython_SGD:
    ...
    def __init__(self, URM, ICM,
        ...
        URM = check_matrix(URM, 'csr').astype(int)
        self.URM_mask_indices = np.array(URM.indices, dtype=int)
        self.URM_mask_indptr = np.array(URM.indptr, dtype=int)

        ICM = check_matrix(ICM, 'csr').astype(int)
        self.ICM_indices = np.array(ICM.indices, dtype=int)
        self.ICM_indptr = np.array(ICM.indptr, dtype=int)
        ...
        if weights_initialization_D is not None:
            ...
            self.D = np.array(weights_initialization_D, dtype=float)
        else:
            self.D = np.zeros(self.n_features, dtype=float)
        ...
        if weights_initialization_V is not None:
            ...
            self.V = np.array(weights_initialization_V, dtype=float)
        else:
            self.V = np.random.normal(mean_init, std_init, (self.n_factors, self.n_features)).astype(float)
        ...
        if self.precompute_user_feature_count:
            ...
            URM.data = np.ones_like(URM.data, int)
            user_feature_count = (URM * ICM).astype(int)

            user_feature_count = check_matrix(user_feature_count, 'csr')
            self.user_feature_count_indices = np.array(user_feature_count.indices, dtype=int)
            self.user_feature_count_indptr = np.array(user_feature_count.indptr, dtype=int)
            self.user_feature_count_data = np.array(user_feature_count.data, dtype=int)
            ...
            self.user_feature_count_counter_temp = np.zeros(self.n_features, dtype=int)
            self.user_feature_count_flag = np.zeros(self.n_features, dtype=int)

        self.user_feature_count_len = 0
        self.user_feature_count_id = np.zeros(self.n_features, dtype=int)
        self.user_feature_count_counter = np.zeros(self.n_features, dtype=int)
        ...
        if sgd_mode=='adagrad':
            ...
            self.sgd_cache_D = np.zeros((self.n_features), dtype=float)
            self.sgd_cache_V = np.zeros((self.n_factors, self.n_features), dtype=float)
            ...
            self.useAdam = True
            self.sgd_cache_D_momentum_1 = np.zeros((self.n_features), dtype=float)
            self.sgd_cache_D_momentum_2 = np.zeros((self.n_features), dtype=float)

            self.sgd_cache_V_momentum_1 = np.zeros((self.n_factors, self.n_features), dtype=float)
            self.sgd_cache_V_momentum_2 = np.zeros((self.n_factors, self.n_features), dtype=float)
    ...
    def fit(self):
        ...
        cdef int [:] user_feature_count_u_full_vector = np.zeros(self.n_features, dtype=int)
        cdef double [:] D_update = np.zeros(self.n_features, dtype=float)
        cdef double [:,:] V_update = np.zeros((self.n_factors, self.n_features), dtype=float)
        cdef int [:] updated_features_id = np.zeros(self.n_features, dtype=int)
        cdef int [:] updated_features_flag = np.zeros(self.n_features, dtype=int)
        cdef int updated_features_count

        # cdef array[double] template_zero = array('d')
        # cdef array[double] V_f_u, Vdelta_ij, Vfi, Ddelta_ij
        cdef double [:] V_f_u = np.zeros(self.n_factors, dtype=float)
        cdef double [:] Vdelta_ij = np.zeros(self.n_factors, dtype=float)
        cdef double [:] Vfi = np.zeros(self.n_factors, dtype=float)
        cdef double [:] Ddelta_ij = np.zeros(self.n_features, dtype=float)
