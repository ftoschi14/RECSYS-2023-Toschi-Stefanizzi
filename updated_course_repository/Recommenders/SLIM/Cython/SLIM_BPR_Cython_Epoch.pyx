...
cdef class SLIM_BPR_Cython_Epoch:
    ...
    cdef long[:] URM_mask_indices, URM_mask_indptr
    ...
    def __init__(self, URM_mask,
        ...
        self.URM_mask_indices = np.array(URM_mask.indices, dtype=np.int64)
        self.URM_mask_indptr = np.array(URM_mask.indptr, dtype=np.int64)
        ...
        if self.train_with_sparse_weights:
            ...
            self.S_dense = np.zeros((self.n_items, self.n_items), dtype=float)
    ...
    def _init_adaptive_gradient_cache(self, sgd_mode, gamma, beta_1, beta_2):
        ...
        if sgd_mode=='sgd':
            ...
            # Adagrad and RMSProp
            self.sgd_cache_I = np.zeros((self.n_items), dtype=np.double)

            # Adam
            self.sgd_cache_I_momentum_1 = np.zeros((self.n_items), dtype=np.double)
            self.sgd_cache_I_momentum_2 = np.zeros((self.n_items), dtype=np.double)
        ...
        if self.topK == False:
            ...
            if self.train_with_sparse_weights:
                ...
                if self.final_model_sparse_weights:
                    ...
                    return np.array(self.S_dense, dtype=float)
    ...
    cdef float adaptive_gradient(self, float gradient, long user_or_item_id, double[:] sgd_cache, double[:] sgd_cache_momentum_1, double[:] sgd_cache_momentum_2):
...
cdef class Triangular_Matrix:
    ...
    cdef get_scipy_csr(self, long TopK = False):
        ...
        cdef np.ndarray[np.float_t, ndim=1] currentRowArray_np
        ...
        for row in range(self.num_rows):
            ...
            if TopK:
                ...
                currentRowArray_np = - np.array(currentRowArray).astype(float)
        ...
        return sps.csr_matrix((data, indices, indptr), shape=(self.num_rows, self.num_cols), dtype=float)
