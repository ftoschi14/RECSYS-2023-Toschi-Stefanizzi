...
cdef class Cosine_Similarity:
    ...
    def __init__(self, URM, TopK = 100):
        ...
        self.user_to_item_data = np.array(URM.data, dtype=float)

        URM = URM.tocsc()
        self.item_to_user_rows = URM.indices
        self.item_to_user_col_ptr = URM.indptr
        self.item_to_user_data = np.array(URM.data, dtype=float)
    ...
    def compute_similarity(self):
        ...
        cdef np.ndarray[float_t, ndim=1] this_item_weights_np
        ...
        cdef int[:] rows = np.zeros((self.n_items*self.TopK,), dtype=int)
        cdef int[:] cols = np.zeros((self.n_items*self.TopK,), dtype=int)
