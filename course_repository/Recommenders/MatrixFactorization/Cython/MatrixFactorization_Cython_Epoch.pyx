...
cdef class MatrixFactorization_Cython_Epoch:
    ...
    def __init__(self, URM_train, n_factors = 1, algorithm_name = None,
        ...
        self.factors_dropout_mask = np.ones(self.n_factors, dtype=int)
    ...
    def _init_latent_factors(self):
        ...
        if self.use_embeddings:
            ...
            self.USER_factors = np.random.normal(self.init_mean, self.init_std_dev, (n_user_factors, self.n_factors)).astype(float)
            self.ITEM_factors = np.random.normal(self.init_mean, self.init_std_dev, (n_item_factors, self.n_factors)).astype(float)

            self.USER_factors_minibatch_accumulator = np.zeros((n_user_factors, self.n_factors), dtype=float)
            self.ITEM_factors_minibatch_accumulator = np.zeros((n_item_factors, self.n_factors), dtype=float)
        else:
            self.USER_factors = np.zeros((n_user_factors, self.n_factors)).astype(float)
            self.ITEM_factors = np.zeros((n_item_factors, self.n_factors)).astype(float)
        ...
        if self.use_bias:
            ...
            self.GLOBAL_bias = np.zeros(1, dtype=float)

            self.USER_bias_minibatch_accumulator = np.zeros(self.n_users, dtype=float)
            self.ITEM_bias_minibatch_accumulator = np.zeros(self.n_items, dtype=float)
            self.GLOBAL_bias_minibatch_accumulator = np.zeros(1, dtype=float)
    ...
    def _init_adaptive_gradient_cache(self, sgd_mode, gamma, beta_1, beta_2):
        ...
        if sgd_mode=='sgd':
            ...
            # Adagrad and RMSProp
            self.sgd_cache_I = np.zeros((self.ITEM_factors.shape[0], self.n_factors), dtype=float)
            self.sgd_cache_U = np.zeros((self.USER_factors.shape[0], self.n_factors), dtype=float)

            self.sgd_cache_bias_I = np.zeros((self.n_items, 1), dtype=float)
            self.sgd_cache_bias_U = np.zeros((self.n_users, 1), dtype=float)
            self.sgd_cache_bias_GLOBAL = np.zeros((1, 1), dtype=float)

            # Adam
            self.sgd_cache_I_momentum_1 = np.zeros((self.ITEM_factors.shape[0], self.n_factors), dtype=float)
            self.sgd_cache_I_momentum_2 = np.zeros((self.ITEM_factors.shape[0], self.n_factors), dtype=float)

            self.sgd_cache_U_momentum_1 = np.zeros((self.USER_factors.shape[0], self.n_factors), dtype=float)
            self.sgd_cache_U_momentum_2 = np.zeros((self.USER_factors.shape[0], self.n_factors), dtype=float)

            self.sgd_cache_bias_I_momentum_1 = np.zeros((self.n_items, 1), dtype=float)
            self.sgd_cache_bias_I_momentum_2 = np.zeros((self.n_items, 1), dtype=float)

            self.sgd_cache_bias_U_momentum_1 = np.zeros((self.n_users, 1), dtype=float)
            self.sgd_cache_bias_U_momentum_2 = np.zeros((self.n_users, 1), dtype=float)

            self.sgd_cache_bias_GLOBAL_momentum_1 = np.zeros((1, 1), dtype=float)
            self.sgd_cache_bias_GLOBAL_momentum_2 = np.zeros((1, 1), dtype=float)
    ...
    def epochIteration_Cython_ASY_SVD_SGD(self):
        ...
        cdef double[:] user_factors_accumulated = np.zeros(self.n_factors, dtype=float)
