...
class SLIMElasticNetRecommender(BaseItemSimilarityMatrixRecommender):
    ...
    def fit(self, l1_ratio=0.1, alpha = 1.0, positive_only=True, topK = 100):
        ...
        URM_train = check_matrix(self.URM_train, 'csc', dtype=float)

        n_items = URM_train.shape[1]

        similarity_builder = Incremental_Similarity_Builder(self.n_items, initial_data_block=self.n_items*self.topK, dtype = float)
...
from multiprocessing import Pool, cpu_count, shared_memory, Lock
...
class MultiThreadSLIM_SLIMElasticNetRecommender(SLIMElasticNetRecommender):
    ...
    def fit(self, alpha=1.0, l1_ratio=0.1, positive_only=True, topK=100,
        ...
        self.URM_train = check_matrix(self.URM_train, 'csc', dtype=float)
        ...
        with Pool(processes=self.workers, initializer=tqdm.set_lock, initargs=(tqdm.get_lock(),)) as pool:
        ...
        self.W_sparse = sps.csr_matrix((values, (rows, cols)), shape=(self.n_items, self.n_items), dtype=float)
