...
cdef class Compute_Similarity_Cython:
    ...
    cdef long[:] this_item_weights_mask, this_item_weights_id
    ...
    cdef long shrink, normalize, adjusted_cosine, pearson_correlation, tanimoto_coefficient, asymmetric_cosine, dice_coefficient, tversky_coefficient
    ...
    def __init__(self, dataMatrix, topK = 100, shrink=0, normalize = True,
        ...
        self.this_item_weights = np.zeros(self.n_columns, dtype=float)
        self.this_item_weights_id = np.zeros(self.n_columns, dtype=np.int64)
        self.this_item_weights_mask = np.zeros(self.n_columns, dtype=np.int64)
        ...
        self.sum_of_squared = np.array(dataMatrix.power(2).sum(axis=0), dtype=float).ravel()
        ...
        if row_weights is not None:
            ...
            self.row_weights = np.array(row_weights, dtype=float)
        ...
        self.user_to_item_data = np.array(dataMatrix.data, dtype=float)
        ...
        self.item_to_user_data = np.array(dataMatrix.data, dtype=float)
    ...
    cdef applyPearsonCorrelation(self, dataMatrix):
        ...
        sumPerCol = np.array(dataMatrix.sum(axis=0), dtype=float).ravel()
    ...
    cdef applyAdjustedCosine(self, dataMatrix):
        ...
        sumPerRow = np.array(dataMatrix.sum(axis=1), dtype=float).ravel()
    ...
    def compute_similarity(self, start_col=None, end_col=None):
        ...
        cdef np.ndarray[np.long_t, ndim=1] relevant_items_partition
        cdef np.ndarray[np.float_t, ndim=1] this_item_weights_np = np.zeros(self.n_columns, dtype=float)
        #cdef double[:] this_item_weights
        ...
        cdef long[:] rows = np.zeros((max_cells,), dtype=np.int64)
        cdef long[:] cols = np.zeros((max_cells,), dtype=np.int64)
